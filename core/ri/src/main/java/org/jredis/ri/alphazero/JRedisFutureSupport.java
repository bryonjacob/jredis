...
//	@Override
	public Future<List<byte[]>> zrangebyscore(String key, double minScore, double maxScore, Opt... opts) {
		byte[] keybytes = null;
		if((keybytes = getKeyBytes(key)) == null) 
			throw new IllegalArgumentException ("invalid key => ["+key+"]");
		...
		return new FutureByteArrayList(this.queueRequest(Command.ZRANGEBYSCORE$OPTS, keybytes, minScoreBytes, maxScoreBytes, Opts.toSpecBytes(Command.ZRANGEBYSCORE$OPTS, opts)));
	}
